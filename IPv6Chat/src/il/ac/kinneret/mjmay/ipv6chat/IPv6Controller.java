...
public class IPv6Controller implements Initializable {
     ...
     * @param event Ignored
     */
    public void toggleListen(ActionEvent event) throws IOException {
        // if we're not listening, then listen
    ...
    public void sendMessage(ActionEvent event) {
        ...
        String otherIP = tfOtherIP.getText();
        int otherPort = Integer.parseInt(tfOtherPort.getText());
        String message = tfMessage.getText();

        try {
            SendTask.sendMessage(otherIP, otherPort, message);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
